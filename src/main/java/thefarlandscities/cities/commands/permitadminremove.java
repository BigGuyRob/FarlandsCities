...
public class permitadminremove implements CommandExecutor {

    private Cities plugin;
    private MongoClient mongoClient;
    private Document dbcity;

    public permitadminremove(Cities plugin, MongoClient mongoClient) {
        this.plugin = plugin;
            ...
            // do the command structure
                ...
                if (target != null && target!=player) {
                    ...
                    if(!permits.contains(target.getName())){
                        player.sendMessage("Player must have a permit before being removed as an admin");
                    }else {
                        if (admins.contains(target.getName())) {
                            admins.remove(target.getName());
                            updateDB(dbcity, collection, admins);
                            player.sendMessage("You have removed " + target.getName() + "as an admin in " + dbcity.get("label"));
                        } else {
                            player.sendMessage(target.getName() + " is not an admin in " + dbcity.get("label"));
                        }
                    }
                } else {
                    player.sendMessage("The second arguement must be an online player name and you cannot remove yourself as an admin");
                }
        }else{
            //Player does not have "permission" to use this command
            player.sendMessage("You cannot remove admins from " + dbcity.get("label"));
        }
        return true;
        }
