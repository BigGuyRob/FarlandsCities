...
public class depermit implements CommandExecutor {

    private Cities plugin;
    private MongoClient mongoClient;
    private Document dbcity;

    public depermit(Cities plugin, MongoClient mongoClient) {
        this.plugin = plugin;
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (admins.contains(player.getName())) {
            // do the command structure
                ...
                if (target != null && target!=player && !admins.contains(player.getName())) {
                    List<String> permits = (List<String>) dbcity.get("permits");
                    if(permits.contains(target.getName())){
                        permits.remove(target.getName());
                        updateDB(dbcity, collection, permits);
                        player.sendMessage("You have removed a permit from " + target.getName());
                    }else{
                        player.sendMessage("This player does not have a permit to in " + dbcity.get("label"));
                    }
                } else {
                    player.sendMessage("The second arguement must be an online player name and you cannot remove permits from yourself");
                }
        }else{
            //Player does not have "permission" to use this command
            player.sendMessage("you do not have permission to remove permits from players in " + dbcity.get("label"));
        }
        return true;
        }
